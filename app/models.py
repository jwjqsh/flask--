...
class User(db.Model):
    ...
    phone = db.Column(db.String(100), unique=True)  # 手机
    info = db.Column(db.Text)  # 简介
    face = db.Column(db.String(255), unique=True)  # 头像
    addtime = db.Column(db.DateTime, index=True, default=datetime.utcnow)  # 注册时间
    uuid = db.Column(db.String(255), unique=True)  # 标准
...
class Movie(db.Model):
    ...
    pyaynum = db.Column(db.BigInteger)  # 播放量
...
class Moviecol(db.Model):
    __tablenme__ = "moviecol"
    id = db.Column(db.Integer, primary_key=True)  # 编号
    movie_id = db.Column(db.Integer, db.ForeignKey("movie.id"))  # 收藏电影
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'))  # 收藏用户
    addtime = db.Column(db.DateTime, index=True, default=datetime.utcnow)  # 添加时间
...
class Role(db.Model):
    ...
    addtime = db.Column(db.DateTime, index=True, default=datetime.utcnow)  # 添加
